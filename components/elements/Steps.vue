<template>
  <section class="steps-wrap">
    <div class="model-choose-text">
      <h1 v-if="car_model">
        Подберите выгодные условия на LADA {{ car_model }} {{ car_type }} в
        {{ $store.state.city.dative }}
      </h1>
      <p v-else-if="!car_model">
        Подберите выгодные условия на LADA в {{ $store.state.city.dative }}
      </p>
    </div>
    ...
    <div class="car-choose_wrap" v-if="!car_model">
      <div class="dropdown-group">
        <v-select
          id="select_auto"
          class="select_wrap event"
          :searchable="false"
          :multiple="false"
          :options="car_list"
          placeholder="Выберите автомобиль"
          taggable
          v-model="selected_auto"
    ...
    <div class="buy-steps-wrapper">
      <div class="buy-step-block">
        <div
          :id="prefix + 'steps__rate-a-car'"
          class="divided event"
          v-on:click.prevent="gradeShow(0)"
          :class="[
            grade == 0 || grade == 1
      ...
      <div class="buy-step-block">
        <div
          :id="prefix + 'steps__сalculate-payment'"
          class="divided event"
          v-on:click.prevent="gradeShow(2)"
          :class="[
            grade == 2 || grade == 3
              ? 'buy-step-circle-colored'
              : 'buy-step-circle'
      ...
      <div class="buy-step-block">
        <div
          :id="prefix + 'steps__fill-form'"
          class="buy-step-circle divided event"
          v-on:click.prevent="gradeShow(4)"
    ...
    <div class="progressbar-wrapper">
      ...
      >
        ...
        <span class="progressbar-text" v-if="grade === 1">
          Мы готовы выкупить ваш автомобиль на 10% дороже рынка при обмене на
          новенькую LADA
        </span>
    ...
    <div class="option-text step-null" v-if="!grade">
      ...
      <ul>
        <li>
          <img
            loading="lazy"
            src="/images/icons/checkbox-green.svg"
            class="check check-green"
            alt="Обмен для всех. Принимаем автомобили любых марок и в любом состоянии."
          />
          <span>
        ...
        <li>
          <img
            ...
            src="/images/icons/checkbox-green.svg"
            class="check check-green"
            alt="У нас все честно. Справедливая цена и никаких скрытых комиссий."
          />
          <span>
        ...
        <li>
          <img
            ...
            src="/images/icons/checkbox-green.svg"
            class="check check-green"
            alt="АВТОВАЗ отменил субсидию при обмене, но мы доплатим от автоцентра до 40 000 рублей"
          />
          <span>
    ...
    <section class="buttons_other mt-20" v-if="!grade">
      <div class="item-buttons-other">
        <a
          href="#"
          :id="prefix + 'do-you-have-an-exchange-car__rate-a-car'"
          class="btn btn-primary event"
          v-on:click.prevent="gradeShow(1)"
          >Да, оценить мой автомобиль</a
        >
        <a
          href="#"
          :id="prefix + 'do-you-have-an-exchange-car__no-car'"
          class="btn btn-secondary event"
          v-on:click.prevent="gradeShow(5)"
          >У меня нет автомобиля</a
        >
      </div>
    </section>
    ...
    <div v-if="grade === 1">
      <div class="button-wrapper-row">
        <a
          :id="prefix + 'what-is-your-car__cash'"
          class="btn btn-half-secondary event"
          v-on:click.prevent="gradeShow(3)"
          >Наличный расчет</a
        >
        <a
          :id="prefix + 'what-is-your-car__credit'"
          class="btn btn-half-primary event"
          v-on:click.prevent="gradeShow(2)"
          >В кредит</a
        >
      </div>
    ...
    <div v-if="grade === 5">
      <div class="button-wrapper-row">
        <a
          :id="prefix + 'сhoose-favorable-conditions__cash'"
          class="btn btn-half-secondary event"
          v-on:click.prevent="gradeShow(3)"
          >Наличный расчет</a
        >
        <a
          :id="prefix + 'сhoose-favorable-conditions__credit'"
          class="btn btn-half-primary event"
          v-on:click.prevent="gradeShow(2)"
          >В кредит</a
        >
    ...
    <div class="" v-if="grade === 3">
      <div class="model-choose-text center" v-if="surchargeText">
        <p>Ваша доплата составит</p>
      </div>
      <div class="your-surcharge" v-if="surchargeText">
        от {{ surcharge | formatPrice }} руб.
      </div>
      <div class="model-choose-text2">
        <p>В Брайт Парке выгодные условия при покупке за наличные!</p>
      </div>
      <div class="conditions">
        <ul>
          ...
          <li>
            <CheckIcon />
            <span>Дополнительное оборудование и аксессуары со скидкой 50%</span>
          </li>
        </ul>
      </div>
    </div>
    <RangeSlider v-if="grade === 2" :car_price="car_price" :prefix="prefix">
    </RangeSlider>
    <FormBuyComponent
      v-if="grade === 4"
      :form_id="form_id"
    ...
    <div class="buttons_other" v-if="grade === 2">
      <div class="item-buttons-other">
        <a
          href="#"
          :id="prefix + 'credit__fix-conditions'"
          class="btn btn-primary event"
          v-on:click.prevent="gradeShow(4)"
          >Закрепить условия</a
        >
    ...
    <div class="buttons_other" v-if="grade === 3">
      <div class="item-buttons-other">
        <a
          href="#"
          :id="prefix + 'cash__fix-conditions'"
          class="btn btn-primary event"
          v-on:click.prevent="gradeShow(4)"
          >Закрепить условия</a
        >
...
export default {
  ...
  methods: {
    ...
    gradeShow: function(grade) {
      ...
      if (
        this.getCookie("trade_in_price") != null &&
        this.getCookie("trade_in_price") > 0
      ) {
    ...
    setCookie(price) {
      ...
      document.cookie =
        "trade_in_price=" + price + ";expires=" + cookieexpire + "path=/";
      return {};
    },
    deleteCookie(name) {
      this.setCookie(0);
...
.progressbar-wrapper {
  ...
  .progressbar-line {
    ...
    &::before {
      ...
      background: url(/images/icons/progressbar.svg) repeat-x,
        linear-gradient(
          90deg,
          rgba(255, 131, 81, 1) 0%,
          rgba(255, 182, 119, 1) 70%
        );
